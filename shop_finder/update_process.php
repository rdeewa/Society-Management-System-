...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_err) && empty($shopname_err) && empty($address_err) && empty($phone_no_err) && empty($work_err) && empty($password_err)) {
        ...
        $sql = "UPDATE shop SET name = ?, shopname = ?, address = ?, phone_no = ?, work = ?, password = ? WHERE id = ?";
        
        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssi", $param_name, $param_shopname, $param_address, $param_phone_no, $param_work, $param_password, $param_id);
            ...
            $param_id = $_POST['id']; // Assuming you have a hidden input field in your form to store the ID
